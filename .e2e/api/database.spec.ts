...
test.describe("Database API", () => {
  test.describe("GET /api/database/heartbeat", () => {
    ...
    test("should return database type information", async ({ request }) => {
      const response = await request.get("/api/database/heartbeat");
      const body = await response.json();
    ...
    test("should return latency information when healthy", async ({ request }) => {
      const response = await request.get("/api/database/heartbeat");
      const body = await response.json();

      if (body.status === "healthy") {
        expect(body).toHaveProperty("latencyMs");
        if (body.latencyMs !== null) {
    ...
    test("should return pool information", async ({ request }) => {
      const response = await request.get("/api/database/heartbeat");
      const body = await response.json();
    ...
    test("should return detail message", async ({ request }) => {
      const response = await request.get("/api/database/heartbeat");
      const body = await response.json();
  ...
  test.describe("Database Connection Pool Details", () => {
    test("should report all configured pools", async ({ request }) => {
      const response = await request.get("/api/database/heartbeat");
      const body = await response.json();

      if (body.pools && body.pools.length > 0) {
        const poolNames = body.pools.map((p: { name: string }) => p.name);
    ...
    test("should include latency for each pool", async ({ request }) => {
      const response = await request.get("/api/database/heartbeat");
      const body = await response.json();
...
test.describe("Database Health Response Format", () => {
  test("should return ISO timestamp format", async ({ request }) => {
    const response = await request.get("/api/database/heartbeat");
    const body = await response.json();
  ...
  test("should include pool health status", async ({ request }) => {
    const response = await request.get("/api/database/heartbeat");
    const body = await response.json();
    ...
    if (body.pools && body.pools.length > 0) {
      const allHealthy = body.pools.every((p: { healthy: boolean }) => p.healthy);
