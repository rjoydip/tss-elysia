...
| `DATABASE_NAME`          | `tss-elysia`   | SQLite database file name                |
...
| `DATABASE_NAME` | `tss-elysia` | Path to SQLite database name |
...
export const env = await _createEnv({
  ...
  runtimeEnv: () => ({
    ...
    DATABASE_NAME: _getEnv("DATABASE_NAME", "tss-elysia"),
...
```bash
# Default location
DATABASE_PATH=.artifacts DATABASE_NAME=tss-elysia bun run db:migrate
...
```bash
DATABASE_PATH=.artifacts DATABASE_NAME=tss-elysia bun run db:migrate
DATABASE_PATH=.artifacts DATABASE_NAME=tss-elysia bun run db:seed
```
