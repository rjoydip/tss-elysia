 ...
 * Modern registration form using TanStack Form for state management.
 * Features OAuth buttons, password strength indicators, and validation.
 */
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md space-y-8">
          ...
          >
                ...
                <div className="space-y-2">
                  ...
                  {field.state.value.length > 0 && (
                    <div className="space-y-2">
                        ...
                        {[1, 2, 3, 4].map((level) => (
                          <div
                            ...
                            className={`h-1 flex-1 rounded-full transition-colors ${
                              level <= getPasswordStrength(field.state.value)
                                ? getStrengthColor(getPasswordStrength(field.state.value))
                                : "bg-muted"
                            }`}
                          />
                        ))}
                      ...
                      <p
                        className={`text-xs ${
                          getPasswordStrength(field.state.value) === 4
                            ? "text-green-600"
                            : "text-muted-foreground"
                        }`}
                      >
                        {getStrengthLabel(getPasswordStrength(field.state.value))}
                      </p>
                    </div>
                  )}
            ...
            {/* Password requirements - only show when user types */}
            <form.Field name="password">
              {(field) =>
                field.state.value.length > 0 && (
                  <div className="space-y-1">
                    {PASSWORD_REQUIREMENTS.map((req, idx) => (
                      <div key={idx} className="flex items-center gap-2 text-xs">
                        ...
                        <span
                          className={
                            req.test(field.state.value) ? "text-green-600" : "text-muted-foreground"
                          }
                        >
                          {req.label}
                        </span>
